...
token = "thr1.AAAAAGk87LBkbK22_PFtEw.WsW-2tdnq5Q"
...
room.onPlayerChat = function (player, message) {
    ...
    if (message[0].toLowerCase() == "!claim"){
        ...
        room.sendChat((highScore[1] + ": Score: " + highScore[0]), player.id);
        return false;
    }else if(message[0].toLowerCase() == "!topstreak"){
        room.sendChat((highStreak[1] + ": Streak: " + highStreak[0]), player.id);
        return false;
    }
}
...
async function startGame() {
    ...
    if(topstreak > highStreak[0]){
        highStreak = [topstreak, player];
