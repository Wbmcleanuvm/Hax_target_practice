...
const room = HBInit({roomName: roomName, maxPlayers: maxPlayers, public: roomPublic, playerName: botName, token: "thr1.AAAAAGk6CvYNWz7n3wwLTQ.EHnte5rJVw4", geo: loc});
...
room.onPlayerJoin = function (player){
    updatePlayers();
    room.sendChat("hi im just testing server pls if u find a bug pls LMK ")
    if(!onRed()){
        room.setPlayerTeam(teamS[1].id, Team.RED)
        room.startGame();
    }
...
room.onPlayerChat = function (player, message) {
    if (message.toLowercase() == "johnnys gay"){
